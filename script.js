...
document.addEventListener("DOMContentLoaded", function () {
    ...
    btnInf.addEventListener('click', function (event) {
        ...
        if (!valor || valor <= 0) {
            msgSpan.style.display = 'inline'; // Exibe a mensagem de erro
        } else {
            msgSpan.style.display = 'none'; // Esconde a mensagem de erro
    
            if (tipoPagamento) {
                if (tipoPagamento.value === 'pix') {
                    pixDiv.style.display = 'block';
                    creditoDiv.style.display = 'none';
                    aplicarDescontoPix(); // Aplica o desconto de 10%
                } else if (tipoPagamento.value === 'cartão-credito') {
                    pixDiv.style.display = 'none';
                    creditoDiv.style.display = 'block';
                    calcularTotalEParcelas(); // Calcula normalmente sem desconto
                }
            }
        }
    });
    ...
    function aplicarDescontoPix() {
        let desconto = valor * 0.10; // 10% de desconto
        let totalComDesconto = valor - desconto;
        valorTotalElem.textContent = `R$${totalComDesconto.toFixed(2)}`;
    }
    ...
    inputNumeroCartao.addEventListener('input', function checkCardNumber() {
        ...
        if (cardNumber.startsWith('1234')) {
            cardIcon.innerHTML = '<i class="fa-brands fa-cc-mastercard"></i>';
            cardIcon.style.display = 'inline';
        } else if (cardNumber.startsWith('4321')) {
            cardIcon.innerHTML = '<i class="fa-brands fa-cc-visa"></i>';
            cardIcon.style.display = 'inline';
